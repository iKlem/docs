...
hexo.extend.tag.register('api_property', function (args) {
	...
	if (counter==0)
		console.log(counter, name, m, this, args);
    var opts = {};
    if(args[2]) {
    ...
    var result = `<h2 id="${id}_${counter++}" data-id="${id}" data-name="${name}" class="api-property api-property--property ${inherited ? 'api-property--inherited' : ''} ${opts.deprecated ? 'api-property--deprecated' : ''}">${inherited}<span class="api-property__name">${name}</span><span class="api-property__type">${args[1]}</span></h2>`;
    if (opts.deprecated) {
